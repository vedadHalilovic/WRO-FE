...
void loop() {
  // 1. Always update the yaw so we don't lose track of orientation
  updateYaw();
  ...
  if (mySerial.available() && !isTurning) {
    ...
    if (data == "L") {
      ...
      // Normal Steering Mode (Expects an integer angle)
...
void updateYaw() {
  ...
  // Convert rad/s to deg/s and remove bias
...
void checkTurnProgress() {
  ...
  if (turnComplete) {
    ...
    // Reset Yaw to 0 AFTER the turn is fully finished
    yaw = 0; 
